...
rule all: 
    input:
        ...
        ### FAI RULE ### 
        # expand(["{spec}" + "/" + "{assemb}__{genome}" + "/" + "genomic.fna.fai"],zip, spec=SPECIES, assemb=ASSEMBLY, genome=GENOME),
...
rule download_assemblies:
    ...
    output:
        out_fna = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "genomic.fna",
        out_gtf = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "genomic.gtf.gz",
...
rule fai_build:
    ...
    input:
        fa = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "genomic.fna"
    output:
        fa = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "genomic.fna.fai",
        dict_in = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "genomic.dict"
    shell:
        """
        set -o xtrace 
...
rule hisat_build:
    ...
    params:
        ...
        target_name = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "hisat2" + "/" + "genomic",
        config_file = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "config.txt",
        master_dir = "/orange/kgraim/panmammalian/Panmammalian/genomes"
    input:
        in_fna = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "genomic.fna" 
    output:
        target = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "hisat2" + "/" + "genomic.1.ht2",
        target2 = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "hisat2" + "/" + "genomic.2.ht2",
        target3 = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "hisat2" + "/" + "genomic.3.ht2",
        target4 = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "hisat2" + "/" + "genomic.4.ht2",
        target5 = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "hisat2" + "/" + "genomic.5.ht2",
        target6 = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "hisat2" + "/" + "genomic.6.ht2",
        target7 = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "hisat2" + "/" + "genomic.7.ht2",
        target8 = "{spec}" + "/" + "{assemb}__{genome}"+ "/" + "hisat2" + "/" + "genomic.8.ht2"
    shell:
        """
...
rule dexseq:
    ...
    params:
        ...
        master_dir = "/orange/kgraim/panmammalian/Panmammalian/genomes"
    input:
        in_gtf = "{spec}" + "/" + "{assemb}__{genome}" + "/" + "genomic.gtf.gz"
    output:
