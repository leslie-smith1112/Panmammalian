...
#export PYTHONPATH=/blue/kgraim/rveernapu/test/disBatch
#export PYTHONPATH=/blue/kgraim/leslie.smith1/disBatch:$PATH
#export PATH=${DISBATCH_ROOT}/disBatch:$PATH
#export PYTHONPATH=/apps/disbatch/2.5
...
do
    ...
    echo "bash -c \"( hisat2  --threads ${Hisat2Cores} -x ${HSX} -1 ${fn1} -2 ${fn2} | samtools view -Sbh -o ${bam} - )\" &> ${outDir}/logs/${bn}_hisat2.log" >> ${outDir}/tasks/hisat2Tasks
...
do
    ...
    echo "python /blue/kgraim/freya/software/DEXSeq/inst/python_scripts/dexseq_count.py -p yes --format bam ${DC_GFF} ${bam} ${outDir}/dexseq_count/${bn}.txt &> ${outDir}/logs/${bn}_dexseq_count.log" >> ${outDir}/tasks/dexTasks
